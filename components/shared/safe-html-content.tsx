...
interface SafeHtmlContentProps {
   ...
   className?: string;
   hasProse?: boolean;
   /** 'sm' = smaller text for compact layouts */
   size?: 'default' | 'sm';
}

function SafeHtmlContent({ content, className = '', hasProse = true, size = 'default' }: SafeHtmlContentProps) {
   if (!content) return <p>No Content Available</p>;

   const sanitized = sanitizeHtmlContent(content);

   if (!hasProse) {
      return <div className={className} dangerouslySetInnerHTML={{ __html: sanitized }}></div>;
   }
   ...
   return (
      <div
         className={cn(
            ...
            'prose-headings:scroll-m-20 prose-headings:font-semibold prose-headings:tracking-tight prose-headings:first:mt-0 prose-headings:mt-8 prose-headings:mb-4',
            size === 'sm'
               ? 'prose-headings:text-xl prose-p:text-sm prose-p:leading-6 prose-p:not-first:mt-5 prose-li:text-sm prose-li:leading-6'
               : 'prose-headings:text-2xl prose-p:text-sm prose-p:leading-6 prose-p:not-first:mt-6 md:prose-p:text-base md:prose-p:leading-7',
            ...
            'prose-a:text-primary prose-a:font-medium prose-a:underline prose-a:underline-offset-4 prose-a:hover:opacity-90',
