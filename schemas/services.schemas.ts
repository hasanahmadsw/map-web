...
export const subServiceSchema = (validationDict: Record<string, string>) =>
  z.object({
    ...
    features: z.array(trimmed(200, validationDict)).min(1, validationDict.selectAtLeastOne || "Please select at least one feature"),
  });
...
export const createServiceSchema = (validationDict: Record<string, string>) =>
  z
    .object({
      ...
      icon: trimmed(100, validationDict).min(1, validationDict.required || "Icon is required"),
      featuredImage: trimmed(500, validationDict).min(1, validationDict.required || "Featured image is required"),
...
export const bulkServiceOperationSchema = (validationDict: Record<string, string>) =>
  z
    .object({
      serviceIds: z.array(z.number().int().positive()).min(1, validationDict.selectAtLeastOne || "Please select at least one service"),
