...
import { AuthorProfileCard } from '@/components/website/authors/author-profile-card';
...
const fakeArticles = [
  {
    ...
    excerpt:
      'World leaders have reached a historic agreement on climate change measures that could significantly impact global emissions.',
  ...
  },
  {
    ...
    excerpt:
      'Major technology companies are preparing for new regulatory frameworks that will reshape the digital landscape.',
    content: 'Full article content here...',
  ...
  },
  {
    ...
    excerpt:
      'The highly anticipated championship finals are set to take place this weekend with record-breaking attendance expected.',
    content: 'Full article content here...',
  ...
  },
  {
    ...
    excerpt:
      'Researchers have announced a promising new treatment that could revolutionize healthcare in the coming years.',
    content: 'Full article content here...',
...
export async function generateMetadata({ params }: AuthorPageProps): Promise<Metadata> {
  ...
  const metaData = createEnhancedMetadata({
    ...
    keywords: [author.name, ...(dict.metadata?.keywords?.authors || [])],
    type: 'website',
...
export default async function AuthorPage({ params }: AuthorPageProps) {
  ...
  const authorArticles = fakeArticles.filter(
    article => article.id <= author.articleCount, // Simple filtering for demo
  );
        ...
        <div className="mx-auto">
          ...
          {authorArticles.length > 0 ? (
            <div>
              ...
              <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
                {authorArticles.map(article => (
                  <NewsCard key={article.id} article={article as any} lang={lang} t={t} variant="vertical" />
                ))}
              </div>
            </div>
          ) : (
            <div className="py-12 text-center">
              ...
              <p className="text-muted-foreground">{t.authors.noArticlesByDescription}</p>
            </div>
          )}
        </div>
      </div>
